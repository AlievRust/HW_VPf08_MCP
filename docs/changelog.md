...
- Добавлено настраиваемое окно истории диалога CLI через `CHAT_HISTORY_LIMIT`.
- CLI теперь передает в модель последние сообщения диалога как контекст следующего запроса.
- Добавлены тесты для загрузки лимита истории, обрезки истории и сборки входного контекста.
